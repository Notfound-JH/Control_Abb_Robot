...
class DobotDemo:
    ...
    def start(self):
        ...
        self.dashboard.EnableSafeSkin(0)  # 设置 TCP 控制模式
        # 启动状态反馈线程
        ...
        # self.dashboard.EnableSafeSkin(0)
        # 定义两个目标点
        # point_a = [146.3759,-283.4321,332.3956,
        #            177.7879,-1.8540,147.5821]
        # point_b = [136.3759,-243.4321,432.3956,177.7879,-1.8540,147.5821]

        # while True:
        #     self.RunPoint(point_b)
        #     self.RunPoint(point_a)  # 走点A
        ...
        #     sleep(2)
    def data_match(self,data):
        matches = re.findall(r'\{([^}]+)\}', data)
        if matches:
            content = matches[0]
            result = [float(num) for num in content.split(',')]
        return result

    def get_data(self):
        joint = self.data_match(self.dashboard.GetAngle())
        pose =  self.data_match(self.dashboard.GetPose())
        return [joint,pose]
    def GetFeed(self):
        # 获取机器人状态
        while True:
